            ...
            while not MESSAGE_QUEUE.empty():
                ...
                if addr not in CLIENTS and addr not in SPECTATORS:
                    if msg_type == "SPECTATOR_REQUEST":
                        ...
                        welcome = "message_type: SPECTATOR_ACCEPTED"
                        self.socket.sendto(welcome.encode(), addr)

                    else:
                        CLIENTS.add(addr)
                        print(f"[HOST] Player joined: {addr}")

                # ----- 2) RESTRICT SPECTATORS -----
                if addr in SPECTATORS:
                    # Spectators can ONLY send CHAT_MESSAGE
                    if msg_type != "CHAT_MESSAGE":
                        print(f"[HOST] Spectator attempted illegal command: {msg_type}")
                        continue  # Ignore battle-related messages

                # ----- 3) BROADCAST TO ALL CLIENTS + SPECTATORS -----
